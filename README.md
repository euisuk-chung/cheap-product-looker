...
5. 공개 Pages는 전체 판매처 중 현재 최저 단가 구매 링크, 최근 확인한 실제 상품 이미지와 가격 변동 이력을 보여 줍니다.
6. 검증할 수 없는 후보는 공개 최저가에 반영하지 않고 추적 현황에 실패 이유를 남깁니다.

상품 상세의 가격 변동 차트는 비교 가능하고 재고가 있었던 실제 관측만 판매처별 선으로 표시합니다. 최신 단위가, 최초 관측 대비 변동폭과 기간 최저 단가를 함께 보여 주며, 관측이 없으면 빈 차트를 합성하지 않습니다.

상품 이미지는 최근 검증 관측의 `imageUrl`을 우선하고, 관측 이미지가 없으면 현재 추적 URL에서 확인한 후보 이미지를 사용합니다. 이미지가 없거나 로드되지 않으면 생성 이미지로 대체하지 않고 다음 수집에서 확인할 대상으로 표시합니다.
